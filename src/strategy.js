...
function evidenceLink(evidence, base) {
  if (!evidence) return '—';
  const file = evidence.file ? `<a class="text-link" href="${REPO_TREE}${base}${esc(evidence.file)}" target="_blank" rel="noreferrer">${esc(String(evidence.file).split('/').pop())} ↗</a>` : '';
...
async function main() {
  ...
  posBody.innerHTML = positions.length ? positions.map((p) => {
    ...
    return `<tr><td><b>${esc(p.title)}</b><br><small>${esc(p.subtitle ? `${p.subtitle} · ` : '')}<a class="text-link" href="${KALSHI_MARKET_URL(p.ticker)}" target="_blank" rel="noreferrer">${esc(p.ticker)} ↗</a></small></td>
      ...
      <td><small>${evidenceLink(p.evidence, base)}</small></td></tr>`;
  ...
  evBody.innerHTML = events.length ? events.map((e) => {
    ...
    return `<tr><td><small>${esc(formatDate(e.at))}<br>${esc(e.cycle)}</small></td>
      ...
      <td><small>${evidenceLink(e.evidence, base)}</small>${e.reason ? `<br><small class="reason" title="${esc(e.reason)}">${esc(e.reason)}</small>` : ''}</td></tr>`;
  ...
  inBody.innerHTML = intents.length ? intents.map((i) => {
    ...
    return `<tr><td><small>${esc(formatDate(i.at))}<br>${esc(i.cycle)}</small></td>
