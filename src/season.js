...
function renderForwardLedger() {
  ...
  if (forward.tab === 'positions') {
    ...
    const injuries = forward.recent?.injuries ?? [];
    const injurySignals = Object.entries(forward.recent?.injurySignals ?? {});
    rows.push(`<tr><td><b>ESPN league injuries · NFL / NBA</b><br><small>drives InjuryFade (opponent quarterback ruled out inside 72h) and TipoffTriage (3+ hard designations on the opponent inside 7 days); ESPN is a third-party feed, never the settlement — Kalshi's official result pays (IRR-43)</small></td><td><small>${injuries.length ? `${injuries.length} snapshot(s) in the window` : 'none in the window'}</small></td><td><small>${injuries.length ? `<a class="text-link" href="${esc(injuries[0].url)}" target="_blank" rel="noreferrer">site.api.espn.com injuries JSON ↗</a><br><small>sha256 ${esc(String(injuries[0].sha256 ?? '').slice(0, 16))}…</small>` : 'site.api.espn.com injuries JSON (public)'}</small></td><td><small>${esc(injuries.map((r) => `${r.league}: ${r.teamCount} club(s) / ${r.playerCount} row(s)`).join(' · ') || 'not needed this cycle')}<br>${esc(injurySignals.length ? injurySignals.slice(0, 3).map(([t, x]) => `${t}: opponent ${x.espnTeam} hard-outs ${x.freshHardOut}${x.freshQbOut ? ` (QB ${x.freshQbOut})` : ''}`).join('; ') : 'no game market had a fresh hard designation')}<br>archived with URL + SHA-256 in signals/espn-injuries.jsonl</small></td></tr>`);
    const nowcast = forward.recent?.nowcast ?? [];
    const nowcastSignals = Object.entries(forward.recent?.nowcastSignals ?? {});
    rows.push(`<tr><td><b>Cleveland Fed inflation nowcast</b><br><small>drives NowcastNudge on KXCPI / KXCPIYOY; the nowcast is a published model estimate — the BLS print settles the market</small></td><td><small>${nowcast.length ? `${nowcast.length} page read(s) in the window` : 'none in the window'}</small></td><td><small>${nowcast.length ? `<a class="text-link" href="${esc(nowcast[0].url)}" target="_blank" rel="noreferrer">clevelandfed.org ↗</a><br><small>sha256 ${esc(String(nowcast[0].sha256 ?? '').slice(0, 16))}…</small>` : 'clevelandfed.org/indicators-and-data/inflation-nowcasting'}</small></td><td><small>${esc(nowcastSignals.length ? nowcastSignals.map(([series, x]) => `${series}: ${x.metric} ${x.label} ${x.value}% (${x.section}, updated ${x.updated ?? '—'})`).join(' · ') : 'no CPI market was open at the cycle time')}<br>verbatim HTML archived in signals/cleveland-fed-nowcast.jsonl</small></td></tr>`);
    const indexCloses = forward.recent?.indexCloses ?? [];
    const indexSignals = Object.entries(forward.recent?.indexSignals ?? {});
    rows.push(`<tr><td><b>FRED official index closes</b><br><small>drives LeapMapper on KXINX / KXNASDAQ100 range markets (persistence test on the last published close, never a forecast)</small></td><td><small>${indexCloses.length ? `${indexCloses.length} CSV read(s) in the window` : 'none in the window'}</small></td><td><small>${indexCloses.length ? `<a class="text-link" href="${esc(indexCloses[0].url)}" target="_blank" rel="noreferrer">fred.stlouisfed.org CSV ↗</a><br><small>sha256 ${esc(String(indexCloses[0].sha256 ?? '').slice(0, 16))}…</small>` : 'fred.stlouisfed.org/graph/fredgraph.csv (key-less)'}</small></td><td><small>${esc(indexCloses.map((r) => `${r.seriesId} ${r.latestDate} ${r.latest}`).join(' · ') || 'not needed this cycle')}<br>${esc(indexSignals.length ? indexSignals.map(([series, x]) => `${series}: close ${x.latest?.value} (${x.latest?.date})`).join(' · ') : 'no index range market was open at the cycle time')}</small></td></tr>`);
    const sourceStatus = forward.recent?.sourceStatus;
    if (sourceStatus?.sources?.length) {
      const failed = (sourceStatus.sources ?? []).filter((r) => r.status === 'failed');
      rows.push(`<tr><td><b>Per-cycle source ledger</b><br><small>every official endpoint the cycle could read, with read / not_needed / failed and the response hash — the machine-readable irregularity flag</small></td><td><small>${esc(sourceStatus.generatedAt ?? '')}</small></td><td><small>forward/sources/status.json</small></td><td><small>${esc((sourceStatus.sources ?? []).map((r) => `${r.source}: ${r.status}`).join(' · '))}${failed.length ? `<br><span class="evidence-pill blocked">${failed.length} failed read(s)</span>` : ''}</small></td></tr>`);
    }
